...
do
  ...
  bash shell_scripts_cv/step1.train.multigpu.sh $ARCH $PRETRAINED_MODEL $BATCH_SIZE $NUM_SAMPLES jhh_train_fold_${i} $i >> $LOG_FILE

  echo "Inference fold $i..."
  bash shell_scripts_cv/step2.inference.multigpu.sh $ARCH out/$ARCH.jhh.fold_$i/model.pth $ARCH.jhh $NUM_GPUS jhh_test_fold_${i} $i >> $LOG_FILE
