...
# Multiple GPUs
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -W ignore -m torch.distributed.launch --nproc_per_node=8 --master_port=$RANDOM_PORT train.py --dist --model_backbone $arch --log_name $arch.jhh.fold_$fold_id --num_class $num_target_class --dataset_path $datapath --num_workers $num_workers --batch_size $batch_size --pretrain $suprem_path --lr 5e-4 --dataset_list $dataset_list --num_samples $num_samples --a_min -100 --a_max 200 --max_epoch 1000 --stage train  #--resume $resume_path

# Wait for the current process to finish
wait

# Sleep for a short time to ensure resources are released
sleep 10
